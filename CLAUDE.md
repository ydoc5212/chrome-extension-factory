...
| `npm run check:cws:ship` | manual | structural + listing/welcome/screenshots/video/privacy content filled in (21 rules; `listing-drift` opt-in on CWS secrets); reads `.factory/ladder-status.json` to gate per-screenshot landings | ✗ red (by design) |
