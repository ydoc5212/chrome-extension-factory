...
- **Privacy policy URL** — must be publicly accessible before submission. Generate and host one in one step: `npm run setup:privacy` (writes `store/PRIVACY.md` from your manifest, enables GitHub Pages via `gh` CLI, wires the URL into welcome config). Self-host with `--self-host=<url>` if you have your own hosting.
...
3. Fill listing fields using templates in `docs/templates/`: [store-listing.md](./templates/store-listing.md), [cws-submission-checklist.md](./templates/cws-submission-checklist.md). For the privacy policy, run `npm run setup:privacy` (auto-generates `store/PRIVACY.md`, hosts on GitHub Pages, wires the URL into welcome config).
