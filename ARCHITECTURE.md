...
| `cws-video` skill (wraps external) | Skill + dependency | **Planned.** Launch-video generation for CWS listings via [`heygen-com/hyperframes`](https://github.com/heygen-com/hyperframes) (install: `npx skills add heygen-com/hyperframes`). Parallel to `cws-screens` — same declarative pattern (config → generate → validate presence), different asset type. Will add a `ship-ready-video` validator rule (ship-only, scoped to fire only when `video/config.ts` exists, similar to how `ship-ready-screenshots` handles the `screenshots/` directory). CWS does not require a promo video, so this rule should stay opt-in — skip the check entirely if `video/` is absent. |
